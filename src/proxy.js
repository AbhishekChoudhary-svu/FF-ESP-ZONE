...
export async function proxy(req) {
  ...
  try {
    ...
    console.log("4. user from DB:", user ? { role: user.role, sessionVersion: user.sessionVersion, banned: user.isBanned, verified: user.emailVerified } : "NOT FOUND")

    if (!user) return NextResponse.redirect(new URL("/login", req.url))

    console.log("5. version check:", user.sessionVersion ?? 1, "vs cookie:", session.sessionVersion)
    ...
    if (!user.emailVerified) {
      console.log("8. REDIRECTING — email not verified")
