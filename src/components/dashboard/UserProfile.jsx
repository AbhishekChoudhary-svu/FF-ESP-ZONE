...
import { CloudUpload, X, Play, User, Lock } from "lucide-react"
...
function GamingCard({ children, className = "" }) {
  return (
    <div className={`relative bg-[#0a0c10] border border-[#2a2e3a] rounded-xl overflow-hidden font-['Rajdhani'] ${className}`}
...
function StatCell({ label, value, accent = false }) {
  return (
    <div className="flex flex-col gap-0.5">
      <span className="text-[11px] text-[#4a5060] uppercase tracking-widest">{label}</span>
      <span className={`text-[15px] font-bold ${accent ? "text-[#ff8c30]" : "text-[#d0d5df]"}`}>
        {value ?? "—"}
      </span>
    </div>
  )
...
function Chip({ children, variant = "orange" }) {
  ...
  return (
    <span className={`text-[12px] font-bold tracking-wider px-3 py-0.5 rounded-[3px] uppercase border ${styles[variant]}`}>
      {children}
    </span>
  )
}
...
function GuestBanner({ onSignup, onLogin }) {
  return (
    <div className="mx-6 mt-5 p-4 rounded-lg border border-[#ff6b00]/30 bg-[#ff6b00]/5
...
function LockedFeature({ children, isGuest, message = "Create an account to use this feature" }) {
    ...
    <div className="relative group cursor-not-allowed">
      ...
      <div className="absolute inset-0 flex items-center justify-center
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 bg-[#0a0c10] border border-[#2a2e3a] rounded-lg">
          ...
          <span className="text-[11px] text-[#8090a0] font-bold uppercase tracking-wider">{message}</span>
        </div>
      </div>
    </div>
  )
...
function UploadZone({ preview, onFile, accept, label, circle = false, disabled = false, onRemove }) {
  ...
  return (
    <div className={`relative flex flex-col items-center justify-center border-2 border-dashed border-[#2a2e3a] ${shape} cursor-pointer hover:border-[#ff6b00]/50 transition bg-[#0f1318]`}>
      {!preview ? (
        <>
          {circle ? <User className="w-6 h-6 text-[#4a5060] mb-1" /> : <CloudUpload className="w-8 h-8 text-[#4a5060] mb-2" />}
...
export function UserProfile() {
  ...
  const isGuest = context?.user?.isGuest || context?.user?.role === "guest"
  ...
  const hasCaptain = String(context?.team?.teamCaptain?._id) === String(context?.player?._id)
  ...
  const inputCls = "w-full bg-[#0f1318] border border-[#2a2e3a] text-[#d0d5df] rounded-md px-3 py-2 text-sm placeholder-[#4a5060] focus:border-[#ff6b00]/50 focus:outline-none transition font-['Rajdhani']"
  ...
  return (
    <>
      ...
      <GamingCard>
        {/* ── HEADER ──────────────────────────────────────────────── */}
        <div className="relative flex justify-between items-start gap-6 px-6 pt-6 pb-5 border-b border-[#1e2330]"
          ...
          <div className="flex gap-5 items-start">
            <div className="relative w-40 h-40 rounded-xl overflow-hidden border-2 border-[#ff6b00]/40 shadow-[0_0_16px_rgba(255,107,0,0.25)] flex-shrink-0 bg-[#1a1f2e] flex items-center justify-center">
              {context?.player?.avatar ? (
                <img src={context.player.avatar} className="w-full h-full object-cover rounded-xl" alt="" />
              ) : (
                <span className="font-['Orbitron'] text-2xl font-black text-[#ff8c30]">
                  {context?.user?.username?.charAt(0)?.toUpperCase() ?? "G"}
                </span>
              )}
              {isGuest && (
            ...
            <div>
              ...
              {context?.user?.bio && !isGuest && (
                <p className="text-[14px] text-[#5a6070] italic max-w-lg">"{context.user.bio}"</p>
              )}
          ...
          <div className="grid grid-cols-2 gap-2.5 flex-shrink-0">
            ...
            <LockedFeature isGuest={isGuest}>
              <DarkDialog open={open} onOpenChange={isGuest ? undefined : setOpen}
                trigger={<GhostBtn disabled={isGuest}>✏ Edit Profile</GhostBtn>}
                title="Edit Profile">
            ...
            <LockedFeature isGuest={isGuest}>
              <DarkDialog open={open1} onOpenChange={isGuest ? undefined : setOpen1}
                trigger={<GhostBtn disabled={isGuest}>{hasPlayer ? "⚙ Edit Player" : "⚔ Register Player"}</GhostBtn>}
                ...
                <form onSubmit={handleSubmitPlayer} className="space-y-5">
                  <div>
                    ...
                    <div className="flex items-center gap-4">
                      <UploadZone preview={avatarPreview} accept="image/*" label="Upload avatar" circle onFile={handleAvatarChange} onRemove={handleDeleteAvatar} />
                  ...
                  <div className="flex items-center gap-5 flex-wrap">
                    ...
                    <label className="flex items-center gap-2 cursor-pointer">
                      <Checkbox checked={isCaptain} onCheckedChange={(v) => setIsCaptain(Boolean(v))}
            ...
            {/* View Clips */}
             <LockedFeature isGuest={isGuest}>
            <DarkDialog open={detailOpen} onOpenChange={isGuest ? undefined : setDetailOpen}
              trigger={
              ...
              {selectedPlayer ? (
                <div className="space-y-4">
                  ...
                  {!selectedPlayer.clipPhotos?.length && !selectedPlayer.clipVideo && (
            ...
            {(!hasTeam || hasCaptain) && (
              <LockedFeature isGuest={isGuest}>
                <DarkDialog open={openTeam} onOpenChange={isGuest ? undefined : setOpenTeam}
                  trigger={<GhostBtn disabled={isGuest}>{hasTeam ? "🛡 Edit Team" : "🛡 Create Team"}</GhostBtn>}
                  ...
                  <form onSubmit={handleSubmitTeam} className="space-y-4">
                    ...
                    <PrimaryBtn type="submit" className="w-full">{hasTeam ? "⚙ Update Team" : "🛡 Create Team"}</PrimaryBtn>
                  </form>
                </DarkDialog>
              </LockedFeature>
            )}

            {/* Team Details — full functionality preserved */}
            {context?.team && (
              <DarkDialog open={teamDetailOpen} onOpenChange={setTeamDetailOpen}
                ...
                {context?.team ? (
                  <div className="space-y-5">
                    ...
                    {/* Members list — fully preserved with kick */}
                    <div>
            ...
            {context?.playerRequests && (
              <LockedFeature isGuest={isGuest}>
              <DarkDialog open={open3} onOpenChange={isGuest ? undefined : setOpen3}
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-[11px] text-[#4a5060]">
                                {req.team?.tag}{isInvite && <span> • {isPending ? "Invited you" : req.status}</span>}
                              </p>
                            </div>
                          </div>
                          <div>
            ...
            {/* Team Join Requests (captain) — fully preserved */}
        ...
        <div className="grid grid-cols-5 border-b border-[#1e2330] bg-[#0c0e14]">
          {[
            { label: "Rank", value: isGuest ? "—" : context?.user?.rank },
            { label: "Playstyle", value: isGuest ? "—" : context?.user?.playstyle },
            { label: "Tournaments", value: isGuest ? "—" : context?.user?.tournamentsJoined },
            { label: "Plan", value: isGuest ? "Guest" : context?.user?.plan?.toUpperCase() },
            { label: "User Type", value: isGuest ? "Guest" : context?.user?.role === "user" ? "Player" : "Moderator" },
          ].map((s, i) => (
            <div key={i} className={`relative px-5 py-3.5 ${i < 4 ? "after:content-[''] after:absolute after:right-0 after:top-[20%] after:bottom-[20%] after:w-px after:bg-[#1e2330]" : ""}`}>
              <StatCell label={s.label} value={s.value} accent={i === 0 && !isGuest} />
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-5 border-b border-[#1e2330] bg-[#0a0c10]">
          {[
            { label: "Team", value: isGuest ? "—" : context?.team?.teamName || "No Team" },
            { label: "Role", value: isGuest ? "—" : context?.player?.inGameRole || "N/A" },
            { label: "Likes", value: isGuest ? "—" : context?.player?.likes || "0" },
            { label: "Captain / IGL", value: isGuest ? "—" : context?.player?.isCaptain ? "Yes" : "No" },
            { label: "Status", value: isGuest ? "—" : context?.player?.isActive ? "Active" : "Inactive" },
          ].map((s, i) => (
            <div key={i} className={`relative px-5 py-3.5 ${i < 4 ? "after:content-[''] after:absolute after:right-0 after:top-[20%] after:bottom-[20%] after:w-px after:bg-[#1e2330]" : ""}`}>
        ...
        <div className="grid grid-cols-5 bg-[#0c0e14]">
          {[
            { label: "Matches", value: isGuest ? "—" : context?.player?.stats?.matchesPlayed || "0" },
            { label: "Win Rate", value: isGuest ? "—" : context?.player?.stats?.winRate ? `${context.player.stats.winRate}%` : "0%" },
            { label: "Kills", value: isGuest ? "—" : context?.player?.stats?.kills || "0" },
            { label: "Assists", value: isGuest ? "—" : context?.player?.stats?.assists || "0" },
            { label: "Deaths", value: isGuest ? "—" : context?.player?.stats?.deaths || "0" },
          ].map((s, i) => (
            <div key={i} className={`relative px-5 py-3.5 ${i < 4 ? "after:content-[''] after:absolute after:right-0 after:top-[20%] after:bottom-[20%] after:w-px after:bg-[#1e2330]" : ""}`}>
              <StatCell label={s.label} value={s.value} accent={i === 2 && !isGuest} />
            </div>
          ))}
        </div>
