...
export function DashboardLayout({ user }) {
  ...
  const handleLogout = async () => {
    try {
      ...
      if (!res.ok) { console.error("Logout failed:", data.error); return; }
...
useEffect(() => {
  ...
  const tabs = [
    { id: "recruitment",      label: "Recruit",   fullLabel: "Recruitment",      icon: "👥" },
    { id: "free-tournaments", label: "Free",      fullLabel: "Free Arena",       icon: "🎮" },
    { id: "paid-tournaments", label: "Paid",      fullLabel: "Paid Matches",     icon: "💰" },
  ...
  const renderTab = () => {
    switch (activeTab) {
      case "recruitment":      return <PlayerRecruitmentTab />;
      case "free-tournaments": return <FreeTournamentsTab />;
      case "paid-tournaments": return <PaidTournamentsTab />;
      case "events":           return <OfficialEventsTab />;
      case "chat":             return <WorldChatTab />;
      default:                 return null;
    }
  };
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#07080b] text-[#d0d5df] font-['Rajdhani'] selection:bg-[#ff6b00]/30 selection:text-white">
        ...
        {/* ── Sticky top nav ───────────────────────────────── */}
        <div className="border-b border-[#1e2330] bg-[#0a0c10]/80 backdrop-blur-md sticky top-0 z-40
        ...
        <div className="max-w-7xl mx-auto px-4 py-8 pb-24 sm:pb-8">
          ...
          <div className="hidden sm:flex gap-2 mb-8">
            {tabs.map((tab) => {
              ...
              return (
                ...
                >
                  <span className={`text-sm ${active ? "scale-110" : "opacity-70"}`}>{tab.icon}</span>
                  {tab.fullLabel}
                </button>
              );
            })}
          ...
          {/* Tab content */}
          <div ref={tabContentRef} className="animate-in fade-in slide-in-from-bottom-2 duration-300">
            {renderTab()}
          </div>
        </div>
        ...
        <nav className="sm:hidden fixed bottom-0 left-0 right-0 z-50 bottom-nav-safe
            ...
            {tabs.map((tab) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Icon */}
                  <span className={`text-lg leading-none transition-transform duration-200 ${active ? "scale-125" : ""}`}>
                    {tab.icon}
                  </span>
