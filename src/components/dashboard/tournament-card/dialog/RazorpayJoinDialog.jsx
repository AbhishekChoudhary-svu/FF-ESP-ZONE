...
export default function RazorpayJoinDialog({ open, onClose, tournament, onSuccess }) {
  ...
  // step: "confirm" | "processing" | "verifying" | "polling" | "done" | "error"
  ...
  const handleClose = useCallback(() => {
    // Don't allow closing mid-verify — could cause confusion
  ...
  const handlePay = useCallback(async () => {
    ...
    try {
        ...
        const rzp = new window.Razorpay({
          ...
          handler: async (response) => {
            ...
            try {
              ...
              // Verify failed — maybe the webhook beat us to it. Poll briefly.
              setStep("polling");
              ...
              // Network error during verify — fall back to polling
              setStep("polling");
              const joinedViaWebhook = await pollForJoin(
                tournament._id,
                String(player?._id),
                { maxAttempts: 6, intervalMs: 2500 }
              );
              if (joinedViaWebhook) {
                resolve({ fromWebhook: true });
              } else {
      ...
      if (err instanceof DOMException && err.name === "AbortError") {
        // User dismissed — reset quietly, no toast
        setStep("confirm");
      } else {
  ...
  }, [tournament, player, onSuccess, onClose]);
        ...
        <div className="p-4 bg-[#07080b] border border-[#ff9a00]/20 rounded-lg space-y-3">
          ...
          <div className="border-t border-[#1e2330] pt-3">
            <p className="text-[11px] text-[#4e5d78] leading-relaxed">
              Secured payment via{" "}
              <span className="text-[#ff9a00] font-bold">Razorpay</span>. UPI, Net
              Banking, Cards &amp; Wallets accepted.
            </p>
          </div>
        </div>
