    ...
    try {
      ...
      if (!res.ok) {
        toast.error(
          ...
          data.error || "Unable to change tournament status",
        );
        return;
      }
      ...
      toast.tournament(
        ...
        `Tournament moved to ${newStatus.toUpperCase()}`,
      );
      ...
      toast.error(
        "SYSTEM ERROR",
        "Something went wrong while updating tournament status",
      );
    } finally {
      setLoading(false);
    }
    ...
    <Dialog open={open} onClose={onClose} title="⚡ Change Status">
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-3 p-3 bg-[#07080b] border border-[#1e2330] rounded-lg">
          <span className="text-[10px] text-[#4e5d78] uppercase tracking-wider font-bold">
            Current:
          </span>
