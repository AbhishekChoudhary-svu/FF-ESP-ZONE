...
export function JoinDialog({ open, onClose, tournament, onSuccess }) {
  ...
  const validateUpi = (val) => {
    ...
    if (!UPI_RE.test(val.trim()))
      return "Invalid format — try yourname@upi or 9876543210@paytm";
    return "";
  };
  ...
  const handleJoin = async () => {
    ...
    if (err) {
      ...
      toast.error("Invalid UPI ID", err);
    ...
    try {
      ...
      if (!upiRes.ok) {
        toast.error(
          "UPI Save Failed",
          upiData.error || "Failed to save UPI ID",
        );
        return;
      }
      ...
      if (!res.ok) {
        toast.error("Join Failed", data.error || "Failed to join tournament");
        return;
      }

      toast.tournament(
        "Registration Successful",
        `You have joined ${tournament.name}`,
      );
    ...
    >
      <div className="space-y-5">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={onClose} className={cls.btnGhost}>
            Cancel
          </button>
        </div>
      </div>
  ...
  return (
    <div className="flex items-center gap-3 p-3 bg-[#07080b] border border-[#1e2330] rounded-lg">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-bold text-[#d0d5df] truncate">
