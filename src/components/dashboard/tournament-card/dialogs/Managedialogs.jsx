...
export function EditTournamentDialog({ open, onClose, tournament, onSuccess }) {
  ...
  const handleChange = (e) => {
    ...
    setForm((p) => ({
      ...p,
      [name]: name === "prizePool" ? Number(value) : value,
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.registrationDeadline || !form.startDate || !form.endDate) {
      const message = "All date fields are required";
      setError(message);
    ...
    if (new Date(form.registrationDeadline) >= new Date(form.startDate)) {
      const message = "Registration deadline must be before start date";
      setError(message);

      toast.error("Invalid Dates", message);
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        const message = data.error || "Failed to update tournament";
      ...
      {!canEdit ? (
        <div className="p-4 bg-[#07080b] border border-red-900/30 rounded-lg text-center space-y-2">
          ...
          <p className="text-xs text-[#4e5d78]">
            Only draft and upcoming tournaments can be edited.
          </p>
        </div>
      ) : (
...
export function RoomCredentialsDialog({
  open,
  onClose,
  tournament,
  onSuccess,
  ...
  const handleSubmit = async (e) => {
    ...
    if (!roomId.trim() || !roomPassword.trim()) {
      toast.error(
        "Missing Credentials",
        "Both Room ID and Room Password are required",
      );
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(
          "Publish Failed",
          data.error || "Failed to publish room credentials",
        );
        return;
      }
      ...
      toast.room(
        ...
        "Players can now access the Room ID and Password",
      );
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="p-4 bg-[#07080b] border border-[#ff9a00]/20 rounded-lg">
          <p className="text-xs text-[#ff9a00] font-bold uppercase tracking-wider">
            ⚠️ Credentials are visible to all registered participants
            immediately.
          </p>
        </div>
        ...
        {tournament?.roomId && (
          <div className="p-3 bg-[#4ade80]/5 border border-[#4ade80]/20 rounded-lg">
            <p className="text-[11px] text-[#4ade80] font-bold uppercase tracking-wider">
              ✅ Already published — updating will overwrite existing
              credentials
            </p>
          </div>
        )}
...
function Pill({ color, children }) {
  ...
  return (
    <span
      className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded border ${colors[color] ?? colors.gray}`}
    >
      {children}
    </span>
  );
