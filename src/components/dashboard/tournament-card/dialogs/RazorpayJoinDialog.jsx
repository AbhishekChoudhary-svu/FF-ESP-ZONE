...
export default function RazorpayJoinDialog({
  open,
  onClose,
  tournament,
  onSuccess,
  ...
  const handlePay = useCallback(async () => {
    ...
    try {
      ...
      if (!upiRes.ok) {
        ...
        toast.error(
          "UPI SAVE FAILED",
          d.error || "Unable to save your payout UPI ID",
        );
      ...
      await new Promise((resolve, reject) => {
        const rzp = new window.Razorpay({
          ...
          modal: {
            ondismiss: () =>
              reject(new DOMException("User dismissed checkout", "AbortError")),
          },
      ...
      toast.payment(
        "PAYMENT VERIFIED",
        "Tournament entry confirmed successfully",
      );
      ...
      if (err instanceof DOMException && err.name === "AbortError") {
        ...
        toast.payment("PAYMENT FAILED", message);
      }
    }
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            {
              label: "Mode",
              value: `${tournament?.gameMode} ${tournament?.teamMode}`,
              color: "text-[#63b3ed]",
            },
            {
              label: "Prize",
              value: `₹${tournament?.prizePool?.toLocaleString() ?? 0}`,
            ...
            <div
              key={i}
              className="p-3 bg-[#07080b] border border-[#1e2330] rounded-lg text-center"
            >
              <p className="text-[10px] text-[#4e5d78] uppercase tracking-wider font-bold mb-1">
                {s.label}
              </p>
              <p className={`text-sm font-black font-['Orbitron'] ${s.color}`}>
                {s.value}
              </p>
            </div>
          ))}
          ...
          <div className="space-y-2">
            {joiningMembers.length > 0 ? (
              joiningMembers.map((m, i) => (
                <div
                  key={i}
                  className="flex items-center gap-3 p-3 bg-[#07080b] border border-[#1e2330] rounded-lg"
                >
                  <div className="w-8 h-8 rounded-lg overflow-hidden border border-[#2a2e3a] bg-[#1a1f2e] flex items-center justify-center flex-shrink-0">
                    {m?.avatar ? (
                      ...
                      <span className="text-xs font-black text-[#ff8c30]">
                        {m?.userId?.username?.charAt(0) ?? "?"}
                      </span>
                    )}
                  </div>
                  <div className="flex-1">
                    <p className="text-sm font-bold text-[#d0d5df]">
        ...
        <div className="p-4 bg-[#07080b] border border-[#1e2330] rounded-lg space-y-3">
          ...
          <input
            ...
            onBlur={() => {
              if (upiId && !UPI_RE.test(upiId.trim()))
                setUpiError(
                  "Invalid format — try yourname@upi or 9876543210@paytm",
                );
            }}
          ...
          {upiError ? (
            ...
            <p className="text-[11px] text-[#4e5d78]">
              Format: <span className="text-[#a78bfa]">name@bankhandle</span> or{" "}
              <span className="text-[#a78bfa]">mobile@upi</span>
            </p>
          )}
        </div>
        ...
        <div className="p-4 bg-[#07080b] border border-[#ff9a00]/20 rounded-lg space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-sm text-[#8090a0] font-semibold">
              Entry Fee
            </span>
            <span className="font-['Orbitron'] font-black text-[#ffaa00]">
              ₹{tournament?.entryFee}
            </span>
          </div>
          <div className="border-t border-[#1e2330] pt-3">
            <p className="text-[11px] text-[#4e5d78] leading-relaxed">
              Secured payment via{" "}
              <span className="text-[#ff9a00] font-bold">Razorpay</span>. UPI,
              Net Banking, Cards &amp; Wallets accepted.
            </p>
          </div>
        </div>
        ...
        {step === "done" && (
          <div className="flex items-center justify-center gap-3 py-4 bg-[#4ade80]/10 border border-[#4ade80]/30 rounded-lg">
            ...
            <span className="text-[#4ade80] font-bold uppercase tracking-wider text-sm">
              Successfully Joined!
            </span>
          </div>
        )}
        {step === "polling" && (
          <div className="flex items-center justify-center gap-3 py-3 bg-[#63b3ed]/10 border border-[#63b3ed]/20 rounded-lg">
            ...
            <span className="text-[#63b3ed] font-bold uppercase tracking-wider text-xs">
