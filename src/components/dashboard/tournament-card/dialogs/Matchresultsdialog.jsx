...
function calcPoints(gameMode, teamMode, row) {
  if (gameMode === "CS") {
    ...
    return { placementPoints: matchPts, killPoints: roundPts, total: matchPts + roundPts }
  }
  ...
  if (gameMode === "CS") {
    return (
      <div className="p-4 bg-[#07080b] border border-[#1e2330] rounded-lg">
        <p className="text-[10px] font-bold uppercase tracking-widest text-[#4e5d78] mb-3">◆ CS Point System</p>
        <div className="grid grid-cols-2 gap-3 text-xs">
          <div className="flex justify-between">
            <span className="text-[#8090a0]">Match Win</span>
            <span className="font-black font-['Orbitron'] text-[#4ade80]">3 pts</span>
          </div>
          <div className="flex justify-between">
            <span className="text-[#8090a0]">Match Loss</span>
            <span className="font-black font-['Orbitron'] text-red-400">0 pts</span>
          </div>
          <div className="flex justify-between">
            <span className="text-[#8090a0]">Each Round Won</span>
            <span className="font-black font-['Orbitron'] text-[#ffaa00]">+1 pt</span>
          </div>
        </div>
      </div>
    )
      ...
      <div className="grid grid-cols-2 gap-x-6 gap-y-1">
        {entries.map(([place, pts]) => (
          <div key={place} className="flex justify-between text-xs">
            ...
            <span className="font-black font-['Orbitron'] text-[#ffaa00]">{pts} pts</span>
          </div>
        ))}
        <div className="flex justify-between text-xs col-span-2 border-t border-[#1e2330] pt-1 mt-1">
          ...
          <span className="font-black font-['Orbitron'] text-[#63b3ed]">+1 pt</span>
        </div>
      </div>
    </div>
  )
...
function BRRow({ row, index, onUpdate, totalParticipants, teamMode }) {
  ...
  const inputCls = "w-full px-2 py-1.5 bg-[#07080b] border border-[#1e2330] rounded text-[#d0d5df] text-xs font-bold focus:outline-none focus:border-[#ff6b00]/60 transition-colors font-['Orbitron'] text-center"

  return (
    <div className={`p-3 bg-[#07080b] border rounded-lg space-y-3 ${row.isDisqualified ? "border-red-900/40 opacity-60" : "border-[#1e2330]"}`}>
      {/* Name + DQ toggle */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          {row.teamLogo || row.avatar ? (
            <img src={row.teamLogo || row.avatar} className="w-7 h-7 rounded object-cover border border-[#2a2e3a]" alt="" />
          ) : (
            <div className="w-7 h-7 rounded bg-[#1a1f2e] border border-[#2a2e3a] flex items-center justify-center">
              <span className="text-[10px] font-black text-[#ff8c30]">{row.name?.charAt(0)}</span>
            </div>
          )}
          <div>
        ...
        <div className="flex items-center gap-2">
          <span className="font-['Orbitron'] font-black text-sm text-[#ffaa00]">{pts.total} pts</span>
          <label className="flex items-center gap-1 cursor-pointer">
            <input
              type="checkbox"
              checked={row.isDisqualified}
              onChange={(e) => onUpdate(index, "isDisqualified", e.target.checked)}
              className="w-3.5 h-3.5 accent-red-500"
            />
            <span className="text-[10px] text-red-400 font-bold uppercase tracking-wider">DQ</span>
          </label>
        </div>
      </div>
      ...
      <div className="grid grid-cols-5 gap-2">
        <div>
          <p className="text-[9px] text-[#4e5d78] uppercase tracking-wider font-bold mb-1 text-center">Place</p>
          <input
            type="number" min="1" max={totalParticipants} placeholder="#"
            value={row.placement}
        ...
        <div>
          <p className="text-[9px] text-[#4e5d78] uppercase tracking-wider font-bold mb-1 text-center">Kills</p>
          <input
            type="number" min="0"
            value={row.kills}
        ...
        <div>
          <p className="text-[9px] text-[#4e5d78] uppercase tracking-wider font-bold mb-1 text-center">Assists</p>
          <input
            type="number" min="0"
            value={row.assists}
        ...
        <div>
          <p className="text-[9px] text-[#4e5d78] uppercase tracking-wider font-bold mb-1 text-center">Deaths</p>
          <input
            type="number" min="0"
            value={row.deaths}
        ...
        <div>
          <p className="text-[9px] text-[#4e5d78] uppercase tracking-wider font-bold mb-1 text-center">Prize ₹</p>
          <input
            type="number" min="0"
            value={row.prize}
      ...
      <div className="flex gap-3 text-[10px] font-bold">
        <span className="text-[#4e5d78]">
          Place: <span className="text-[#ffaa00]">{pts.placementPoints}pts</span>
        </span>
        <span className="text-[#4e5d78]">
  ...
  return (
    <div className={`p-3 bg-[#07080b] border rounded-lg space-y-3 ${row.isDisqualified ? "border-red-900/40 opacity-60" : "border-[#1e2330]"}`}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          {row.teamLogo ? (
            <img src={row.teamLogo} className="w-7 h-7 rounded object-cover border border-[#2a2e3a]" alt="" />
          ) : (
            <div className="w-7 h-7 rounded bg-[#1a1f2e] border border-[#2a2e3a] flex items-center justify-center">
              <span className="text-[10px] font-black text-[#ff8c30]">{row.name?.charAt(0)}</span>
            </div>
      ...
      <div className="grid grid-cols-5 gap-2">
        {[
          ...
          <div key={fi}>
            <p className="text-[9px] text-[#4e5d78] uppercase tracking-wider font-bold mb-1 text-center">{f.label}</p>
            <input
              type="number" min="0"
              value={row[f.field]}
      ...
      <div className="flex gap-3 text-[10px] font-bold">
        <span className="text-[#4e5d78]">
          Match pts: <span className="text-[#ffaa00]">{pts.placementPoints}pts</span>
        </span>
        <span className="text-[#4e5d78]">
...
export function MatchResultsDialog({ open, onClose, tournament, onSuccess }) {
  ...
  const sortedByPoints = [...rows]
    ...
    .sort((a, b) => calcPoints(gameMode, teamMode, b).total - calcPoints(gameMode, teamMode, a).total)
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isCS) {
      ...
      if (missing.length > 0) {
        toast.error("Enter placement for all non-disqualified participants")
        return
      }
    }
    ...
    try {
      ...
      toast.success("Results submitted! Player stats updated. 🏆")
      onSuccess?.()
      onClose()
  ...
  return (
    <Dialog open={open} onClose={onClose} title="📊 Submit Match Results" maxWidth="max-w-3xl">
      {fetching ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="p-3 bg-[#07080b] border border-[#63b3ed]/20 rounded-lg">
            <p className="text-xs text-[#63b3ed] font-bold uppercase tracking-wider">
              ℹ️ {existing ? "Updating existing results." : "First time submitting."} Submitting marks tournament completed and auto-updates all player stats, win rates and tournament history.
            </p>
          </div>
            ...
            <div className="space-y-3">
              ...
              {rows.map((row, i) =>
                isCS ? (
                  ...
                  <BRRow key={i} row={row} index={i} onUpdate={updateRow} totalParticipants={rows.length} teamMode={teamMode} />
                )
              )}
            </div>
          )}
            ...
            <div className="p-4 bg-[#07080b] border border-[#1e2330] rounded-lg">
              ...
              <div className="space-y-1.5">
                {sortedByPoints.map((row, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center justify-between px-3 py-2 bg-[#0a0c10] border border-[#1e2330] rounded-lg">
                      <div className="flex items-center gap-3">
                        <span className={`font-['Orbitron'] font-black text-sm w-6 ${
                          i === 0 ? "text-[#ffaa00]" :
                          i === 1 ? "text-[#8090a0]" :
                          i === 2 ? "text-[#cd7f32]" : "text-[#4e5d78]"
                        }`}>
                          #{i + 1}
                        </span>
                        <span className="text-sm font-bold text-[#d0d5df]">{row.name}</span>
                      </div>
                      <div className="flex items-center gap-4 text-xs font-bold">
                        {!isCS && (
                          <>
                            <span className="text-[#4e5d78]">
                              Place <span className="text-[#ffaa00]">{pts.placementPoints}p</span>
                            </span>
                            <span className="text-[#4e5d78]">
                              Kills <span className="text-[#63b3ed]">{pts.killPoints}p</span>
                            </span>
                          </>
                        )}
                        ...
                        {row.prize > 0 && (
                          <span className="text-green-400">₹{Number(row.prize).toLocaleString()}</span>
                        )}
                      </div>
                    </div>
                  )
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {loading
                ...
                : existing ? "📊 Update Results" : "📊 Submit Final Results"}
            </button>
            <button
              type="button"
              onClick={onClose}
