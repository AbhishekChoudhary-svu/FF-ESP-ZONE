...
export function EditPlayerDialog({
  ...
  const handleSubmit = async (e) => {
    ...
    if (!avatar) {
      toast.error(
        "Avatar Required",
        "Please upload a player avatar before continuing",
      );
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(
          hasPlayer ? "Update Failed" : "Creation Failed",
      ...
      toast.team(
        hasPlayer ? "Player Profile Updated" : "Player Profile Created",
        hasPlayer
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex items-center gap-5 flex-wrap">
          ...
          <CheckboxField
            checked={isCaptain}
            onChange={(v) => setIsCaptain(Boolean(v))}
            label="Captain"
            checkedColor="bg-[#ff6b00] border-[#ff6b00]"
          />
          <CheckboxField
            checked={isActive}
            onChange={(v) => setIsActive(Boolean(v))}
            label="Active"
            checkedColor="bg-[#4ade80] border-[#4ade80]"
          />
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <MultiPhotoZone
              ...
              onChange={handleClipPhotosChange}
            />
