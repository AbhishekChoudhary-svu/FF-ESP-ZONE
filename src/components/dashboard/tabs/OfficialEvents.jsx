    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 bg-black/85 backdrop-blur-sm">
      ...
      <div className="relative w-full sm:max-w-2xl bg-[#0a0c10] border border-[#2a2e3a] sm:rounded-xl rounded-t-2xl overflow-hidden shadow-[0_-4px_40px_rgba(0,0,0,0.7)] sm:shadow-[0_10px_40px_rgba(0,0,0,0.7)] font-['Rajdhani']">
        ...
        <div className="px-4 sm:px-6 py-4 sm:py-5 max-h-[65svh] overflow-y-auto space-y-4 ff-event-scroll">
          ...
          {event.bannerImage && (
            <img src={event.bannerImage} alt={event.title} className="w-full h-36 sm:h-40 object-cover rounded-lg border border-[#1e2330]" />
          )}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
              {event.prizePool > 0 && (
                <div className="p-3 bg-[#07080b] border border-[#1e2330] rounded-lg text-center">
                  <p className="text-[9px] sm:text-[10px] text-[#4e5d78] uppercase tracking-wider font-bold mb-1">Prize Pool</p>
                  <p className="text-sm sm:text-base font-black font-['Orbitron'] text-green-400">₹{event.prizePool.toLocaleString()}</p>
                </div>
              )}
              {event.entryFee > 0 && (
                <div className="p-3 bg-[#07080b] border border-[#1e2330] rounded-lg text-center">
                  <p className="text-[9px] sm:text-[10px] text-[#4e5d78] uppercase tracking-wider font-bold mb-1">Entry Fee</p>
                  <p className="text-sm sm:text-base font-black font-['Orbitron'] text-[#ff9a00]">₹{event.entryFee.toLocaleString()}</p>
                </div>
              )}
              {event.startDate && (
                <div className="col-span-2 sm:col-span-1 p-3 bg-[#07080b] border border-[#1e2330] rounded-lg text-center">
                  <p className="text-[9px] sm:text-[10px] text-[#4e5d78] uppercase tracking-wider font-bold mb-1">Starts</p>
                  <p className="text-[11px] sm:text-xs font-bold text-[#d0d5df]">{formatDate(event.startDate)}</p>
                </div>
              )}
            </div>
          )}
            ...
            <div>
              ...
              <div className="space-y-2">
                {event.prizeDistribution.map((p, i) => (
                  <div key={i} className="flex items-center justify-between px-3 sm:px-4 py-2 sm:py-2.5 bg-[#07080b] border border-[#1e2330] rounded-lg">
                    <div className="flex items-center gap-2 sm:gap-3">
                      <span className="font-['Orbitron'] font-black text-sm text-[#ffaa00]">#{p.placement}</span>
                      <span className="text-xs sm:text-sm text-[#8090a0] font-semibold truncate max-w-[120px]">{p.description}</span>
                    </div>
                    <span className="font-['Orbitron'] font-black text-sm text-green-400 flex-shrink-0">₹{p.prize?.toLocaleString()}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 sm:gap-3">
              {event.startDate && (
                <div className="p-3 bg-[#07080b] border border-[#1e2330] rounded-lg">
                  <p className="text-[9px] sm:text-[10px] text-[#4e5d78] uppercase tracking-wider font-bold mb-1">Start Date</p>
                  <p className="text-[11px] sm:text-xs font-bold text-[#d0d5df]">{formatDate(event.startDate)}</p>
                </div>
              )}
              {event.endDate && (
                <div className="p-3 bg-[#07080b] border border-[#1e2330] rounded-lg">
                  <p className="text-[9px] sm:text-[10px] text-[#4e5d78] uppercase tracking-wider font-bold mb-1">End Date</p>
                  <p className="text-[11px] sm:text-xs font-bold text-[#d0d5df]">{formatDate(event.endDate)}</p>
                </div>
              )}
            </div>
          )}
          ...
          {event.tags?.length > 0 && (
            <div className="flex flex-wrap gap-1.5 sm:gap-2">
              {event.tags.map((tag, i) => (
                <span key={i} className="text-[10px] font-bold uppercase tracking-wider px-2 sm:px-2.5 py-1 bg-[#ff6b00]/10 border border-[#ff6b00]/20 text-[#ff8c30] rounded">
                  #{tag}
                </span>
              ))}
            </div>
          ...
          <div className="flex items-center justify-between pt-2 border-t border-[#141822] text-[11px] text-[#4e5d78] font-bold uppercase tracking-wider flex-wrap gap-2">
            <span className="truncate">By {event.organizerName || event.organizer?.username}</span>
            <span className="flex items-center gap-1 flex-shrink-0"><Eye className="w-3.5 h-3.5" /> {event.viewCount} views</span>
          </div>
        </div>
...
function EventFormDialog({ initial, onClose, onSuccess, mode = "create" }) {
  ...
  const inputCls  = "w-full px-3 py-2.5 bg-[#07080b] border border-[#1e2330] rounded-lg text-[#d0d5df] placeholder-[#4e5d78] text-sm font-semibold focus:outline-none focus:border-[#ff6b00]/60 transition-colors"
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 bg-black/85 backdrop-blur-sm">
      ...
      <div className="relative w-full sm:max-w-2xl bg-[#0a0c10] border border-[#2a2e3a] sm:rounded-xl rounded-t-2xl overflow-hidden shadow-[0_-4px_40px_rgba(0,0,0,0.7)] sm:shadow-[0_10px_40px_rgba(0,0,0,0.7)] font-['Rajdhani']">
        ...
        <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-[#141822] bg-[#0d0f15]">
          ...
          <button onClick={onClose} className="text-[#4e5d78] hover:text-red-400 active:text-red-400 transition-colors p-1 -mr-1">
            <X className="w-5 h-5" />
          </button>
        </div>

        {/* Form */}
        <form onSubmit={handleSubmit} className="px-4 sm:px-6 py-4 sm:py-5 max-h-[80svh] overflow-y-auto ff-event-scroll space-y-4">
          ...
          <div>
            ...
            <textarea name="description" value={form.description} onChange={handleChange} required placeholder="Full details..." rows={3} className={`${inputCls} resize-none`} />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" name="prizePool" value={form.prizePool} onChange={handleChange} min="0" inputMode="numeric" className={`${inputCls} text-green-400 font-['Orbitron'] font-bold`} />
            </div>
            <div>
              <label className={labelCls}>Entry Fee (₹)</label>
              <input type="number" name="entryFee" value={form.entryFee} onChange={handleChange} min="0" inputMode="numeric" className={`${inputCls} text-[#ff9a00] font-['Orbitron'] font-bold`} />
            </div>
          </div>
          ...
          <div>
            ...
            <input name="bannerImage" value={form.bannerImage} onChange={handleChange} placeholder="https://..." className={inputCls} inputMode="url" autoCapitalize="none" />
          </div>

          <div>
            <label className={labelCls}>Registration Link</label>
            <input name="registrationLink" value={form.registrationLink} onChange={handleChange} placeholder="https://..." className={inputCls} inputMode="url" autoCapitalize="none" />
          </div>
          ...
          <div className="flex gap-3 pt-1 pb-2">
            ...
            >
              {loading
                ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> {isEdit ? "Saving..." : "Creating..."}</>
...
export function OfficialEventsTab() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.ok) { setDeletingEvent(null); fetchEvents() }
      else { const data = await res.json(); alert(data.error || "Failed to delete event") }
    } catch {
  ...
  const selectCls = "appearance-none px-3 py-2 bg-[#07080b] border border-[#1e2330] rounded-lg text-xs font-bold uppercase tracking-wider text-[#8090a0] focus:outline-none focus:border-[#ff6b00]/60 cursor-pointer transition-colors"
    ...
    <>
      ...
      <div className="space-y-4 sm:space-y-6 font-['Rajdhani'] text-[#d0d5df]">
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3 bg-[#0a0c10] p-3 border border-[#1e2330] rounded-xl">
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className={`${selectCls} flex-1 min-w-[130px]`}>
            <option value="all">📡 All Status</option>
            <option value="upcoming">Upcoming</option>
            <option value="ongoing">Ongoing</option>
            <option value="ended">Ended</option>
          </select>
          <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className={`${selectCls} flex-1 min-w-[130px]`}>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mb-1.5 flex-wrap">
                      ...
                      <span className={`text-[9px] sm:text-[10px] font-bold tracking-widest px-2 py-0.5 rounded border uppercase ${statusStyle(event.status)}`}>
                        {event.status}
                      </span>
      ...
      {showCreate     && <EventFormDialog mode="create" onClose={() => setShowCreate(false)} onSuccess={fetchEvents} />}
      {editingEvent   && <EventFormDialog mode="edit" initial={editingEvent} onClose={() => setEditingEvent(null)} onSuccess={fetchEvents} />}
      {deletingEvent  && <DeleteConfirmDialog event={deletingEvent} loading={deleteLoading} onClose={() => setDeletingEvent(null)} onConfirm={handleDelete} />}
    </>
  )
}