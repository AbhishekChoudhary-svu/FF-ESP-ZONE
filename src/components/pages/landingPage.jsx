    ...
    <div className="min-h-screen bg-[#07080b] text-[#d0d5df] font-['Rajdhani'] selection:bg-[#ff6b00]/30 selection:text-white relative">
      ...
      <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-[#ffaa00]/30 to-transparent" />
        ...
        <div className="relative z-10 mx-auto max-w-4xl text-center">
          ...
          <p className="mb-10 mx-auto max-w-2xl text-balance text-base text-[#8090a0] sm:text-lg font-medium tracking-wide">
            Deploy open custom brackets, lock down top-tier tournament prize modules, 
            and source competitive roster units based on historical data analytics.
          </p>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="text-xs font-semibold">
              {[
                ...
                <tr key={idx} className="border-b border-[#141822]/60 hover:bg-[#141822]/20 transition-colors">
                  <td className="p-4 font-bold text-white uppercase tracking-wide">{item.title}</td>
                  <td className="p-4 text-[#8090a0] font-['Orbitron'] text-[11px]">{item.format}</td>
                  <td className="p-4 text-[#ffaa00] font-['Orbitron'] font-bold">{item.prize}</td>
                  <td className="p-4 text-[#8090a0]">{item.slots}</td>
                  <td className="p-4 text-right">
                    <span className={`inline-block px-2 py-0.5 border text-[10px] font-['Orbitron'] font-bold uppercase rounded-sm ${
                      item.live ? "bg-[#ff6b00]/10 border-[#ff6b00]/30 text-[#ff6b00]" : "bg-[#ffaa00]/10 border-[#ffaa00]/30 text-[#ffaa00]"
                    }`}>
                      {item.status}
                    </span>
                  </td>
      ...
      <section className="px-4 py-12 mx-auto max-w-6xl text-center">
        ...
        <p className="text-xs text-[#8090a0] uppercase tracking-widest mb-6">
          Adjust sandbox parameters to simulate competitive team progression pathways
        </p>
      </section>
            ...
            {[
              {
                ...
                description: "Deploy and compete within regional battle grounds with completely neutralized entry-ticket constraints.",
                icon: <Users className="h-5 w-5 text-[#ff6b00]" />,
              },
              {
                title: "Paid Tournaments",
                description: "High-stakes high-yield parameters structured for verified profiles seeking locked prize allocations.",
                icon: <Trophy className="h-5 w-5 text-[#ffaa00]" />,
              },
              {
                title: "Team Recruitment",
                description: "Incorporate targeted search functions to filter prospective squads by position and tactical KDA indices.",
                icon: <Zap className="h-5 w-5 text-[#ff6b00]" />,
              },
              {
                title: "Anti-Cheat Guard",
                description: "Real-time verification handshakes paired with dynamic coordinate telemetry tracking infrastructure.",
            ...
            <div className="md:col-span-2 rounded-lg border bg-[#0a0c10] border-[#1e2330] p-6 flex flex-col justify-between shadow-inner">
              <div>
                ...
                <p className="text-xs text-[#8090a0] mb-6 font-semibold">
                  Deploy matches across multiple standard configurations natively linked to automatic custom room handlers.
                </p>
              </div>

              <div className="space-y-4">
                <div>
                  <div className="text-[11px] font-['Orbitron'] font-bold text-white mb-2 tracking-wider">BATTLE ROYALE (BR)</div>
                  <div className="flex flex-wrap gap-2">
                    {["Solo Matrix", "Duo Sync", "Squad Deployment"].map((mode) => (
                      <span key={mode} className="rounded-sm px-2.5 py-1 text-xs font-['Orbitron'] font-bold border uppercase tracking-wider bg-[#ff6b00]/5 border-[#ff6b00]/20 text-[#ff6b00]">
                        {mode}
                      </span>
                    ))}
                  </div>
                ...
                <div>
                  <div className="text-[11px] font-['Orbitron'] font-bold text-white mb-2 tracking-wider">CLASH SQUAD (CS)</div>
                  <div className="flex flex-wrap gap-2">
                    {["1v1 Terminal", "2v2 Skirmish", "4v4 Core Shock"].map((mode) => (
                      <span key={mode} className="rounded-sm px-2.5 py-1 text-xs font-['Orbitron'] font-bold border uppercase tracking-wider bg-[#ffaa00]/5 border-[#ffaa00]/20 text-[#ffaa00]">
                        {mode}
                      </span>
                    ))}
                  </div>
            ...
            <div className="rounded-lg border bg-[#0a0c10] border-[#1e2330] p-6 flex flex-col justify-between shadow-inner">
              <div>
                ...
                <p className="text-xs text-[#8090a0] mb-4 font-semibold">
                  Decentralized liquidity scaling criteria for standard battle units:
                </p>
              </div>
              ...
              <div className="space-y-3 border-t border-[#141822]/60 pt-4 text-xs font-semibold uppercase tracking-wider">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[#ff9a00] font-['Orbitron'] font-bold">50% Share</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[#8090a0]">Runner-Up Unit</span>
                  <span className="text-white font-['Orbitron']">30% Share</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[#8090a0]">Third Place Squad</span>
                  <span className="text-white font-['Orbitron']">20% Share</span>
                </div>
        ...
        <div className="mx-auto max-w-6xl">
          <div className="grid gap-8 text-center md:grid-cols-3">
            {[
              ...
              <div key={item.label} className="p-4 border border-[#141822] bg-[#07080b]/50 rounded">
                <div className="mb-1 text-3xl font-black font-['Orbitron'] text-[#ff9a00] tracking-wider">
                  {item.stat}
                </div>
                <div className="text-xs font-bold text-[#4e5d78] uppercase tracking-widest">{item.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mx-auto max-w-6xl">
          <div className="mb-10 grid gap-8 grid-cols-2 md:grid-cols-4">
            <div className="col-span-2 md:col-span-1">
              <div className="mb-3 font-['Orbitron'] font-black text-white tracking-wider text-sm">FF-ESP-ZONE</div>
            ...
            {[
              ...
              <div key={col.title}>
                <h4 className="mb-4 text-xs font-bold text-white uppercase tracking-widest">{col.title}</h4>
                <ul className="space-y-2 text-xs font-semibold uppercase tracking-wider">
                  {col.links.map((link) => (
                    <li key={link}>
                      <a href="#" className="text-[#4e5d78] hover:text-[#ff9a00] transition-colors">
                        {link}
                      </a>
                    </li>
                  ))}
