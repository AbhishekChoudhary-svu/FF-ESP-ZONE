...
export function CreateTournamentForm({ onClose, userRole }) {
  ...
  const getSlotInfo = () => {
    ...
    if (formData.teamMode === "Squad")
      return { slots: 12, perSlot: 4, total: 48 };
    if (formData.teamMode === "Duo")
      return { slots: 24, perSlot: 2, total: 48 };
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      toast.error("Missing Dates", "All date fields are required");
      return;
    }
    ...
    if (formData.tournamentType === "paid" && formData.entryFee <= 0) {
      toast.error(
        "Invalid Entry Fee",
        "Paid tournaments must have an entry fee greater than 0",
      );
      return;
    }
    ...
    try {
      ...
      if (formData.tournamentType === "free") {
      ...
      const isPublished =
        ...
        formData.initialStatus === "upcoming";

      if (isPublished) {
        toast.tournament(
          "Tournament Published",
          "Players can now register for the tournament",
        );
      } else {
        toast.tournament("Draft Saved", "Tournament has been saved as draft");
      }
      ...
      toast.error("Creation Error", err.message || "Something went wrong");
  ...
  const inputCls =
    "w-full px-3 py-2 bg-[#07080b] border border-[#1e2330] rounded text-[#d0d5df] placeholder-[#4e5d78] text-sm font-semibold tracking-wide focus:outline-none focus:border-[#ff6b00]/60 transition-colors";
  const labelCls =
    "block text-xs font-bold uppercase tracking-wider text-[#8090a0] mb-2";
  const selectCls =
    "w-full appearance-none px-3 py-2 bg-[#07080b] border border-[#1e2330] rounded text-[#d0d5df] text-sm font-bold uppercase tracking-wider focus:outline-none focus:border-[#ff6b00]/60 cursor-pointer transition-colors";
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          <label className={labelCls}>
            Tournament Type <span className="text-[#ff6b00]">*</span>
          </label>
          <div className="grid grid-cols-2 gap-3">
            ...
            <button
              type="button"
              onClick={() =>
                !canCreatePaid ? null : set("tournamentType", "paid")
              }
            ...
            >
              💰 Paid
              {!canCreatePaid && (
                <span className="text-[9px] ml-1 normal-case">
                  (admin only)
                </span>
              )}
            </button>
          ...
          <div>
              ...
              {PAID_STATUS_OPTIONS.map((opt) => (
                <button
                  ...
                  className={`flex flex-col items-start px-4 py-3 rounded border transition-all text-left ${
                    formData.initialStatus === opt.value
                      ? opt.active
                      : opt.color
                  }`}
                >
            ...
            {formData.initialStatus === "draft" && (
              <p className="text-[11px] text-[#ff9a00]/70 mt-2 font-semibold">
                ⚠️ Draft tournaments are invisible to players until you publish
                them via the ⚡ Change Status button on the card.
              </p>
            )}
          </div>
        )}
        ...
        <div>
          ...
          <textarea
            name="description"
            placeholder="Tournament overview, format, rewards..."
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="relative">
            <label className={labelCls}>
              Game Mode <span className="text-[#ff6b00]">*</span>
            </label>
            <select
              name="gameMode"
            ...
            <div className="absolute top-[34px] right-3 pointer-events-none text-[10px] text-[#4e5d78]">
              ▼
            </div>
          </div>
          <div className="relative">
            <label className={labelCls}>
              Team Mode <span className="text-[#ff6b00]">*</span>
            </label>
            <select
              name="teamMode"
        ...
        <div className="grid grid-cols-4 divide-x divide-[#1e2330] bg-[#07080b] border border-[#1e2330] rounded-lg overflow-hidden">
          {[
            ...
            {
              label: "Mode",
              value: formData.gameMode === "CS" ? "4v4" : formData.teamMode,
              color: "text-[#63b3ed]",
            },
          ].map((s, i) => (
            <div
            ...
            >
              <p className="text-[9px] text-[#4e5d78] uppercase tracking-wider font-bold mb-1">
                {s.label}
              </p>
              <p
                className={`text-base font-black font-['Orbitron'] ${s.color}`}
              >
                {s.value}
              </p>
            </div>
        ...
        <div>
          ...
          <textarea
            name="rules"
            placeholder="Match rules, ping limits, code of conduct..."
        ...
        <div className="p-4 bg-[#07080b] border border-[#1e2330] rounded-lg">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-1.5 text-[12px]">
            {[
              ...
              {
                label: "Mode",
                value: `${formData.gameMode} — ${formData.teamMode}`,
                color: "text-[#d0d5df]",
              },
              {
                label: "Capacity",
                value: `${slotInfo.total} players (${slotInfo.slots} slots)`,
                color: "text-[#d0d5df]",
              },
              {
                label: "Entry Fee",
              ...
              <div key={i} className="contents">
                ...
                <span className={`font-bold uppercase ${s.color}`}>
                  {s.value}
                </span>
              </div>
            ))}
